...
const appLoginReadWrite = async (req, res) => {
  ...
  try {
    ...
    console.error('Error obtaining token with read/write permissions:', error);
    res.status(500).json({ error: 'Unable to obtain access token with read/write permissions.' });
  }
};
